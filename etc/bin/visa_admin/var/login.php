...
function login(){
	...
	$.ajax({
        type: "POST",
        dataType: "json",
        url: "<?php echo functions::get_Config('webCog')['site'];?>?a=visa_admin&b=api&c=login&csrf=" + csrf,
        data: $('#from').serialize(),
        success: function (data) {
            if(data.code == '200'){
            	layer.closeAll('loading');
            	layer.msg(data.msg, {icon: 1});
            	setTimeout(function(){location.href = '<?php echo functions::urlc('visa_admin','index','home');?>';},2000);
            }else{
            	layer.closeAll('loading');
