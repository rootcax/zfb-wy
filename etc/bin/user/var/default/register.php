...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	...
	<title>免费注册 - <?php echo functions::get_Config('webCog')['title'];?></title>
...
<!-- 						<div class="form-group"> 
							<div class="alert alert-success" role="alert">感谢注册<?php echo functions::get_Config('webCog')['title'];?></div>
			</div> -->
						<div class="form-group">
							...
							<input type="text" class="form-control" id="phone" name="phone" placeholder="手机号" autocomplete="off">
                                                ...
                                                <?php $sms_config = functions::get_Config('smsCog');  if($sms_config['register_sms']){?>
						<div class="form-group">
							<div class="row">
							<div class="col-md-8">
						...
						<div class="form-group">
							...
							<input type="password" class="form-control" name="pwd" placeholder="设置密码" autocomplete="off">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<input type="button" value="立即注册" class="btn btn-primary" style="width: 100%;" onclick="register();">
						</div>
					</form>
					<!-- END Sign In Form -->
...
function sendemail(){
	...
	var csrf = $('#csrf').val();
	layer.load();
	 $.get("<?php echo functions::get_Config('webCog')['site'];?>?a=user&b=api&c=sms&typec=1&phone=" + phone + "&csrf=" + csrf, function(result){
		    if(result.code == '200'){
...
function register(){
	...
	$.ajax({
        type: "POST",
        dataType: "json",
        url: "<?php echo functions::get_Config('webCog')['site'];?>?a=user&b=api&c=register&csrf=" + csrf,
        data: $('#from').serialize(),
