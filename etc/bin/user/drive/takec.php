...
class takec {
    ...
    function del($user) {
        ...
        if (empty($id))
            functions::urlx(functions::urlc('user', 'index', 'takes'));
        $row = '';
        for ($i = 0; $i < count($id); $i++) {
            $row .= "id={$id[$i]} or ";
        }
        $row = trim(trim($row), 'or');
        $delete = functions::open_mysql()->delete('takes', "{$row} and userid={$user->sid}");
        functions::urlx(functions::urlc('user', 'index', 'takes'));
    }
    ...
    function request($user) {
        ...
        $take = $mysql->query("takes", "id={$orderId} and userid={$userid}");
        $take = $take[0];
        //查询用户
