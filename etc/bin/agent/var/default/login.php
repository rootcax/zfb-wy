...
function login(){
	...
	$.ajax({
        type: "POST",
        dataType: "json",
        url: "<?php echo functions::get_Config('webCog')['site'];?>?a=agent&b=api&c=login&csrf=" + csrf,
        data: $('#from').serialize(),
        success: function (data) {
            if(data.code == '200'){
            	layer.closeAll('loading');
            	layer.msg(data.msg, {icon: 1});
            	setTimeout(function(){location.href = '<?php echo functions::urlc('agent','index','home');?>';},2000);
            }else{
            	layer.closeAll('loading');
